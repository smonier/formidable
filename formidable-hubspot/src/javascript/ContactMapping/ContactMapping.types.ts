/**
 * Where a mapped HubSpot property takes its value from: a form field of the
 * enclosing form, or a constant typed by the contributor.
 */
export type MappingSource = 'field' | 'constant';

/**
 * One line of the mapping table, as stored. This shape is the ONLY contract with the
 * Java side (fieldMapping property of fmdbhs:createContactAction) — keep it exact.
...
/** A Contact property as described by the formidableHubspot.objectFields GraphQL query. */
export interface HubspotField {
    name: string;
    label: string;
    /** HubSpot type: string, number, date, datetime, enumeration, bool. */
    type: string;
    /** HubSpot field type: text, textarea, select, radio, checkbox (multi-value), booleancheckbox, number, date... */
