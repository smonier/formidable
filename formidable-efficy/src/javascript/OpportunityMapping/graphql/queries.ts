...
export const CURRENT_NODE_BY_PATH = gql`
    query EfficyOpportunityMappingCurrentNodeByPath($path: String!, $workspace: Workspace!) {
...
export const FORM_FIELDS_BY_PATH = gql`
    query EfficyOpportunityMappingFormFieldsByPath($path: String!, $workspace: Workspace!, $language: String!) {
        jcr(workspace: $workspace) {
            nodeByPath(path: $path) {
                uuid
                workspace
...
/**
 * Createable Opportunity fields of a Efficy connection, served by the module's own
 ...
 * (the "Refresh Efficy fields" button).
 */
export const OPPORTUNITY_FIELDS = gql`
    query EfficyOpportunityMappingOpportunityFields($connectionId: String!, $contextPath: String!, $refresh: Boolean) {
        formidableEfficy {
