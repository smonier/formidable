...
export const CURRENT_NODE_BY_PATH = gql`
    query SalesforceLeadMappingCurrentNodeByPath($path: String!, $workspace: Workspace!) {
...
export const FORM_FIELDS_BY_PATH = gql`
    query SalesforceLeadMappingFormFieldsByPath($path: String!, $workspace: Workspace!, $language: String!) {
        jcr(workspace: $workspace) {
            nodeByPath(path: $path) {
                uuid
                workspace
...
/**
 * Createable Lead fields of a Salesforce connection, served by the module's own
 ...
 * (the "Refresh Salesforce fields" button).
 */
