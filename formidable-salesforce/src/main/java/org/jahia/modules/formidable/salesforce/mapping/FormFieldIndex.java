    ...
    public static FormFieldIndex build(JCRNodeWrapper actionNode) {
        ...
        try {
            ...
            if (!form.isNodeType(FORM_TYPE)) {
                log.warn("[formidable-salesforce] Action {} is not under a form (found {})", actionNode.getPath(), form.getPath());
                return index;
            }
            ...
            log.warn("[formidable-salesforce] Could not index the form fields of {}: {}", actionNode.getPath(), e.getMessage());
        }
        return index;
    }
